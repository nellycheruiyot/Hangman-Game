...
public class App {
  public static void main(String[] args) {
    ...
    int count = hangman.getWrongLetterCount();
    boolean hang = hangman.getHang();

    while (count<7 && !word.equals(answer)) {
      System.out.println("Guess a letter: ");
      String letter = myConsole.readLine();
      hangman.playGame(letter);
      answer = hangman.getAnswer();
      count = hangman.getWrongLetterCount();
      hang = hangman.getHang();
      // System.out.println(word);
      System.out.println(answer);
      if (hang) {
        printHangman(count, word);
      }
    }
